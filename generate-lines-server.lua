...
local server = assert(socket.bind("183.175.14.101", 8000))
...
while 1 do
  ...
  print("wait for a connection from any client")
  local client = server:accept()
  -- make sure we don't block waiting for this client's line
  client:settimeout(10)
  -- receive the line
  local mline, err = client:receive()
  ...
  for sample in dataset() do
    ...
    for i = 1, 1 do
        ...
        transResu = table.concat(maxattns:squeeze(2):totable(), ' ')
        print('A', transResu)
    end
  end
  -- if there was no error, send it back to the client
  if not err then client:send(transResu .. "\n") end
